...
const FEEDBACK_ENDPOINT =
	(import.meta as unknown as { env?: { VITE_FEEDBACK_ENDPOINT?: string } }).env
...
const exec = (command: string) => {
	try {
		...
		const maybeExec = (doc as { execCommand?: (cmd: string) => boolean })
			.execCommand;
...
export const FeedbackButton: React.FC = () => {
	...
	const handleSubmit = async () => {
		if (!FEEDBACK_ENDPOINT) {
			setMessage(
				'Vui lòng cấu hình FEEDBACK_ENDPOINT (xem docs/FEEDBACK_APPS_SCRIPT.md)'
			);
			return;
		}
		...
		try {
			...
			if (FEEDBACK_ENDPOINT.includes('docs.google.com/forms')) {
				...
				try {
					...
					console.warn(
						'Fetch to Google Form failed (CORS/opaque response). Assuming success.',
						fetchErr
					);
					setMessage('Gửi góp ý thành công — (lưu vào Google Form).');
					try {
						showToast(
							'Gửi góp ý thành công — đã lưu vào Google Form',
							'success'
						);
					} catch (e) {
						console.warn(e);
			...
			if (!res.ok) {
				const maybeMsg =
					jsonData && 'message' in jsonData
						? jsonData.message
						: responseText || `Server returned ${res.status}`;
				const msgStr =
					typeof maybeMsg === 'string' ? maybeMsg : String(maybeMsg);
			...
			try {
				showToast(
					typeof successMsg === 'string' ? successMsg : 'Gửi góp ý thành công',
					'success'
				);
			} catch (e) {
				console.warn(e);
		...
		<>
			...
			<button
				...
				<svg
					...
					<path
						fillRule='evenodd'
						d='M2 15a1 1 0 011-1h3.586l8.707-8.707a4 4 0 015.657 5.657L12.243 19.657A4 4 0 017.586 19H3a1 1 0 01-1-1v-3z'
						clipRule='evenodd'
					/>
				</svg>
			</button>
			...
			{open && (
				<div className='fixed inset-0 z-50 flex items-center justify-center'>
					...
					<div className='relative bg-white dark:bg-slate-800 rounded-lg w-full max-w-2xl mx-4 p-6 shadow-2xl border border-gray-200 dark:border-slate-700'>
						...
						<label className='block text-sm font-medium text-gray-700 dark:text-gray-200 mt-3'>
							Email (tuỳ chọn)
						</label>
						<input
							value={email}
							...
							<div className='flex items-center justify-between'>
								...
								<div className='flex items-center gap-2'>
									<button
										...
										onClick={() => exec('bold')}
										title='Đậm'
										className='px-2 py-1 text-sm rounded bg-gray-100 dark:bg-slate-700'>
										B
									</button>
									<button
										type='button'
