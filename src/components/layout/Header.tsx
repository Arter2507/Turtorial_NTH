...
import { SunIcon, MoonIcon, SearchIcon, MenuIcon } from '../common/icons';
					...
					<div className='flex items-center gap-3 min-w-0'>
						{titleIcon &&
							React.cloneElement(titleIcon, {
								className:
									'h-7 w-7 text-sky-600 dark:text-sky-400 hidden sm:block shrink-0',
							})}
				...
				<div className='flex items-center gap-2 sm:gap-4 justify-end'>
					...
					{onOpenSitemap && (
						<button
							...
							className='p-2 rounded-full text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-slate-800'
							aria-label='Open sitemap'>
							{/* Dùng SitemapIcon nếu có, nếu không dùng MenuIcon */}
							{/* <SitemapIcon className='h-6 w-6' /> */}
							<MenuIcon className='h-6 w-6' />
						</button>
					)}
				</div>
